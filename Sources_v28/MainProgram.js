...
MainProgram.prototype.drawScore = function()
{
	...
	if(this.time_max > 0)
	{
		...
		if(this.j_left_shoki <= 0 && this.j_left <= 0)
		{
			s2 = this.moji_score + " " + this.score + "    " + this.moji_highscore + " " + this.highscore + this.moji_time + j;
		} else
		{
			s2 = this.moji_score + " " + this.score + "    " + this.moji_highscore + " " + this.highscore + this.moji_left + this.j_left;
	...
	} else
	{
		...
		var s3;
		if(this.j_left_shoki <= 0 && this.j_left <= 0)
			s3 = this.moji_score + " " + this.score + "    " + this.moji_highscore + " " + this.highscore;
		else
			s3 = this.moji_score + " " + this.score + "    " + this.moji_highscore + " " + this.highscore + this.moji_left + this.j_left;
		...
		if(this.sl_step == 10 || this.sl_step == 11)
		{
			...
			if(this.co_j.c != 300 && this.co_j.c != 310)
				if(this.co_j.x < this.ks_wx - 16)
				{
					...
					if(this.co_j.vx < 0)
						this.co_j.vx = 0;
					if((this.maps.getBGCode(this.co_j.x + 15, this.co_j.y) >= 20 || this.maps.getBGCode(this.co_j.x + 15, this.co_j.y + 31) >= 20) && this.co_j.c >= 100 && this.co_j.c < 200)
					{
						this.co_j.c = 210;
	...
	case 231: 
		...
		if(this.ml_mode_c == 5)
		{
			...
			if(this.ig.dokan_khID == 4)
				...
				s = this.gg.ap.getParameter("url1");
			location.href = s;
		} else
...
MainProgram.prototype.init1 = function()
{
	...
	if(this.grenade_type < 1 || this.grenade_type > 6)
		this.grenade_type = 1;
	...
	if(i == 2)
		this.dengeki_mkf = true;
	else
		this.dengeki_mkf = false;
	i = this.paraInt("yachamo_kf");
	if(i == 2)
		this.yachamo_cf = true;
	else
		this.yachamo_cf = false;
	...
	if(this.boss_type < 1 || this.boss_type > 4)
		this.boss_type = 1;
	this.boss2_type = this.paraInt("boss2_type");
	if(this.boss2_type < 1 || this.boss2_type > 2)
		this.boss2_type = 1;
	this.boss3_type = this.paraInt("boss3_type");
	if(this.boss3_type < 1 || this.boss3_type > 4)
		this.boss3_type = 1;
...
MainProgram.prototype.init3 = function()
{
	...
	var l1;
	if(this.stage == 2)
		l1 = this.paraInt("scroll_mode_s");
	else
	if(this.stage == 3)
		l1 = this.paraInt("scroll_mode_t");
	else
	if(this.stage == 4)
		l1 = this.paraInt("scroll_mode_f");
	else
		l1 = this.paraInt("scroll_mode");
	if(l1 == 2)
	{
		this.sl_step = 10;
		this.ks_wx = 32;
...
MainProgram.prototype.mapsMakeStageData = function(i)
{
	...
	for(var i2 = 0; i2 < this.maps.height; i2++)
		...
		{
			var l2 = as[i2].length;
			for(var j = l2; j < this.maps.width; j++)
				as[i2] += ".";
		...
		for(var k = 0; k < this.maps.width; k++)
		{
			...
			if(c != '.')
			{
				...
				if(c == 'J')
				{
					...
					this.tSet(k * 32 + 80, j2 * 32 - 40, 510, k * 32 - 512 - 32 - 32);
					this.tSet(k * 32 + 140, j2 * 32 + 38, 510, k * 32 - 512 - 32 - 32);
					if(this.maps.map_bg[k - 1][j2] == 4)
						word0 = 4;
				} else
				if(c == 'K')
				{
					...
					if(this.ugokuyuka1_type == 3)
						this.aSet(k * 32, j2 * 32, 510, k * 32);
					else
					if(this.ugokuyuka1_type == 4)
						this.aSet(k * 32, j2 * 32, 120, k * 32);
					else
					if(this.ugokuyuka1_type == 5)
						this.aSet(k * 32, j2 * 32, 121, k * 32);
					else
					if(this.ugokuyuka1_type == 6)
						this.aSet(k * 32, j2 * 32, 600, k * 32);
					else
					if(this.ugokuyuka1_type == 7)
						this.aSet(k * 32, j2 * 32, 700, k * 32);
					else
						this.aSet(k * 32, j2 * 32, 100, k * 32);
					if(this.maps.map_bg[k - 1][j2] == 4)
						word0 = 4;
				} else
				if(c == 'L')
				{
					...
					if(this.ugokuyuka2_type == 3)
						this.aSet(k * 32, j2 * 32, 510, k * 32);
					else
					if(this.ugokuyuka2_type == 4)
						this.aSet(k * 32, j2 * 32, 120, k * 32);
					else
					if(this.ugokuyuka2_type == 5)
						this.aSet(k * 32, j2 * 32, 121, k * 32);
					else
					if(this.ugokuyuka2_type == 6)
						this.aSet(k * 32, j2 * 32, 600, k * 32);
					else
					if(this.ugokuyuka2_type == 7)
						this.aSet(k * 32, j2 * 32, 700, k * 32);
					else
						this.aSet(k * 32, j2 * 32 + 9, 110, k * 32 - 16);
					if(this.maps.map_bg[k - 1][j2] == 4)
						word0 = 4;
				} else
				if(c == 'M')
				{
					...
					if(this.ugokuyuka3_type == 3)
						this.aSet(k * 32, j2 * 32, 510, k * 32);
					else
					if(this.ugokuyuka3_type == 4)
						this.aSet(k * 32, j2 * 32, 120, k * 32);
					else
					if(this.ugokuyuka3_type == 5)
						this.aSet(k * 32, j2 * 32, 121, k * 32);
					else
					if(this.ugokuyuka3_type == 6)
						this.aSet(k * 32, j2 * 32, 600, k * 32);
					else
					if(this.ugokuyuka3_type == 7)
					{
						this.aSet(k * 32, j2 * 32, 700, k * 32);
					} else
					{
				...
				if(c == 'W')
				{
					this.tSet(k * 32, j2 * 32 - 16, 1000, k * 32 - 512 - 32 - 32);
					word0 = 4;
				} else
				if(c == 'X')
				{
...
MainProgram.prototype.drawGamescreen = function()
{
			...
			if(this.co_a[i].gf)
			{
				...
				case 800: 
					if(characterobject.x >= this.co_j.x)
						...
						this.hg.drawImage(this.hih[1][35 + characterobject.c3], j5, i7, this.ap);
					break;
	...
	if(this.m_kazu > 0)
	{
		for(var j = 0; j <= 23; j++)
			if(this.co_m[j].c >= 50)
			{
				...
				{
					this.hg.drawImage(this.hih[characterobject1.pth][characterobject1.pt], characterobject1.x - i3, characterobject1.y - j3, this.ap);
					...
					if(j4 >= 20)
						this.gg.drawPT(((characterobject1.x + 31) >> 5) * 32 - i3, (characterobject1.y >> 5) * 32 - j3, j4, 0);
				} else
				if(characterobject1.pt == 1000)
				{
					this.gg.os_g.setColor(this.gamecolor_mizunohadou);
					this.gg.os_g.fillOval(((characterobject1.x - i3) + 16) - characterobject1.c2, ((characterobject1.y - j3) + 16) - characterobject1.c2, characterobject1.c2 * 2, characterobject1.c2 * 2);
				} else
		...
		for(var k = 0; k <= 1; k++)
			if(this.co_jm[k].c >= 50)
			{
				...
				if(characterobject2.pt < 1000)
					this.hg.drawImage(this.hih[characterobject2.pth][characterobject2.pt], characterobject2.x - i3, characterobject2.y - j3, this.ap);
				else
				if(characterobject2.pt == 1200)
				{
					if(this.g_ac == 0)
						...
						this.gg.os_g.setColor(this.gamecolor_grenade2);
					this.gg.os_g.fillRect(characterobject2.x - i3, (characterobject2.y - j3) + 12, (characterobject2.vx - characterobject2.x) + 1, 8);
				} else
				if(characterobject2.pt == 1205)
				{
					if(this.g_ac == 0)
						...
						this.gg.os_g.setColor(this.gamecolor_grenade2);
					this.gg.os_g.fillRect(characterobject2.vx - i3, (characterobject2.y - j3) + 12, (characterobject2.x - characterobject2.vx) + 1, 8);
				} else
				{
					if(this.g_ac == 0)
						this.gg.os_g.setColor(this.gamecolor_grenade1);
						...
						this.gg.os_g.setColor(this.gamecolor_grenade2);
					this.gg.os_g.fillOval(((characterobject2.x - i3) + 16) - characterobject2.c2, ((characterobject2.y - j3) + 16) - characterobject2.c2, characterobject2.c2 * 2, characterobject2.c2 * 2);
				}
			}
	...
	for(var l = 0; l <= this.t_kazu; l++)
		if(this.co_t[l].c >= 50)
		{
			...
			if(i6 >= -64 && l7 <= 576)
				this.hg.drawImage(this.hih[this.co_t[l].pth][this.co_t[l].pt], i6, l7, this.ap);
		}
	...
	if(this.j_jet_c >= 96)
		if(this.g_c1 == 0)
			this.hg.drawImage(this.hi[134], this.co_j.x - i3, (this.co_j.y - j3) + 36, this.ap);
		else
			this.hg.drawImage(this.hi[135], this.co_j.x - i3, (this.co_j.y - j3) + 36, this.ap);
	if(this.j_v_c > 0)
	{
		this.j_v_c--;
		this.j_v_kakudo += 2;
...
MainProgram.prototype.jM100 = function()
{
	...
	if((i9 == 8 || i9 == 9) && this.maps.map_bg[((this.co_j.x + 15) >> 5) - 1][(this.co_j.y + 15) >> 5] == 4)
	{
		this.j_mizu_f = true;
		this.j_jet_c = 0;
		this.j_mizu_awa_c++;
	...
	} else
	{
		...
		if(this.gk.right_f && !this.j_mizu_f && this.co_j.vx < 60)
		{
			this.co_j.vx += 10;
			if(this.co_j.vx > 60)
				this.co_j.vx = 60;
		...
		if(this.a_hf)
		{
			for(var i4 = 0; i4 <= this.a_kazu; i4++)
				if(this.co_a[i4].gf)
				{
					...
					if(characterobject.c == 700 && this.co_j.x + 15 >= characterobject.x && this.co_j.x <= characterobject.x + 16 && this.co_j.y + 31 >= characterobject.y && this.co_j.y <= characterobject.y + 31)
					{
		...
		if(this.a_hf)
		{
			for(var j4 = 0; j4 <= this.a_kazu; j4++)
				if(this.co_a[j4].gf)
				{
					...
					if(characterobject1.c == 700 && this.co_j.x + 15 >= characterobject1.x && this.co_j.x <= characterobject1.x + 16 && this.co_j.y + 31 >= characterobject1.y && this.co_j.y <= characterobject1.y + 31)
					{
						this.co_j.x = characterobject1.x - 16;
						this.co_j.vx = 0;
					}
	...
	if(this.co_j.jimen_f)
	{
		if(this.tr1_c >= 1 && this.tr1_c <= 5)
		{
			...
			{
				if(this.tr1_c == 1 && this.maps.getBGCode(this.co_j.x + 15, this.co_j.y - 1) == 40)
				{
				...
				if(this.j_helm_f && (i9 == 20 || i9 == 69 && this.suberuyuka_hkf == 1))
				{
					this.tr1_c = 6;
			...
			} else
			{
				...
				} else
				{
					this.co_j.vy = -340;
					this.j_jump_level = 5;
					i9 = this.maps.map_bg[(this.co_j.x + 15) >> 5][(this.co_j.y - 1) >> 5];
					if(i9 <= 9)
					{
	...
	if(!this.j_mizu_f)
	{
		...
		if(this.tr1_c == 0)
			this.j_jet_kf = true;
		if(this.j_jet_kf && this.tr1_c > 0 && this.j_jet_fuel > 0 && !this.j_mizu_f)
		{
			this.j_jet_c = 100;
			if(this.j_jump_type == 2)
				this.j_jump_type = 0;
	...
	} else
	{
		...
		if(this.tr1_c == 1)
		{
			...
			if(i9 <= 9 && i9 != 4)
				flag2 = true;
			if((i9 == 8 || i9 == 9) && this.maps.map_bg[((this.co_j.x + 15) >> 5) - 1][((this.co_j.y + 15) >> 5) - 1] == 4)
				flag2 = false;
	...
	if(this.co_j.vy < 0)
	{
		...
		if(word10 >= 20)
		{
			...
			if(this.j_helm_f && (word10 == 20 || word10 == 69 && this.suberuyuka_hkf == 1))
			{
	...
	if(this.co_j.vy > 0)
	{
		...
		if(!this.j_mizu_f)
		{
			...
			if((j8 == 8 || j8 == 9) && this.maps.map_bg[((this.co_j.x + 15) >> 5) - 1][(this.co_j.y + 15) >> 5] == 4)
			{
				this.mSet(this.co_j.x, ((this.co_j.y + 15) >> 5) * 32 - 32, 50);
				this.j_mizu_awa_c = 38;
				if(this.co_j.vx < -60)
	...
	if(this.tr2_c == 1)
	{
		...
		if(this.j_drell_f && (i9 == 20 || i9 == 69 && this.suberuyuka_hkf == 1))
		{
			var i6 = (this.co_j.x + 15) >> 5;
			var k7 = (this.co_j.y + 32) >> 5;
			if(this.maps.map_bg[(this.co_j.x + 15) >> 5][((this.co_j.y + 32) >> 5) - 1] == 4)
				this.maps.putBGCode(i6, k7, 4);
			else
				this.maps.putBGCode(i6, k7, 0);
...
MainProgram.prototype.jMove = function()
{
	...
	if((k == 8 || k == 9) && this.maps.map_bg[((this.co_j.x + 15) >> 5) - 1][(this.co_j.y + 15) >> 5] == 4)
	{
		this.j_mizu_f = true;
		this.j_jet_c = 0;
		this.j_mizu_awa_c++;
		...
		if(this.stage_cc <= 0)
		{
			...
			if(this.dokan_mode == 2)
			{
				if(this.co_j.c1 >= 10)
				{
					...
					{
						if(this.co_a[i].c != 300 || this.co_a[i].c4 == 1 || this.co_j.c2 != this.co_a[i].c3 || this.sl_step != 0 && this.sl_step != 1 && this.sl_step != 10 && this.sl_step != 11)
							continue;
					...
					for(var j = 0; j <= this.a_kazu; j++)
						...
						{
							this.co_a[j].gf = true;
							if(this.co_a[j].c == 300 && (this.co_j.x != this.co_a[j].x + 16 || this.co_j.y != this.co_a[j].y - 32))
								this.co_a[j].c4 = 0;
						}
	...
	for(var i = 0; i <= this.t_kazu; i++)
		if(this.co_t[i].c != 0 && this.co_t[i].x >= this.maps.wx - 512)
		{
			...
			case 100:
				if(((j + 15) >> 5) > ((j + 12) >> 5))
				{
					var word0 = this.maps.map_bg[(j + 15) >> 5][(k + 31) >> 5];
					if(word0 == 11 || word0 == 14)
						k = ((k + 31) >> 5) * 32 - 32;
					else
					if(word0 == 10 || word0 == 12)
						k = ((k + 31) >> 5) * 32;
				}
				j -= 3;
				...
				} else
				{
					...
					{
						j = (j >> 5) * 32 + 32;
						word1 = this.maps.map_bg[(j + 15) >> 5][(k + 31) >> 5];
			...
			case 105:
				if(((j + 15) >> 5) < ((j + 18) >> 5))
				{
					var word2 = this.maps.map_bg[(j + 15) >> 5][(k + 31) >> 5];
					if(word2 == 10 || word2 == 13)
						k = ((k + 31) >> 5) * 32 - 32;
					else
					if(word2 == 11 || word2 == 15)
						k = ((k + 31) >> 5) * 32;
				}
				j += 3;
				...
				{
					var word3 = this.maps.map_bg[(j + 31) >> 5][(k + 32) >> 5];
					if(word3 <= 9)
					{
						j = ((j + 31) >> 5) * 32 - 32;
						word3 = this.maps.map_bg[(j + 15) >> 5][(k + 31) >> 5];
			...
			case 110:
				if(((j + 15) >> 5) > ((j + 12) >> 5))
				{
					var word4 = this.maps.map_bg[(j + 15) >> 5][(k + 31) >> 5];
					if(word4 == 11 || word4 == 14)
						k = ((k + 31) >> 5) * 32 - 32;
					else
					if(word4 == 10 || word4 == 12)
						k = ((k + 31) >> 5) * 32;
				}
				j -= 3;
				...
				} else
				{
					...
					{
						j = ((j + 31) >> 5) * 32;
						var word5 = this.maps.map_bg[(j + 15) >> 5][(k + 31) >> 5];
						if(word5 >= 10 && word5 <= 15)
							j -= 15;
					...
					} else
					{
						...
						if(word6 >= 20)
						{
							...
							if(this.maps.map_bg[(j + 15) >> 5][(k + 32) >> 5] <= 9)
								characterobject.c = 120;
						}
					}
			...
			case 115:
				if(((j + 15) >> 5) < ((j + 18) >> 5))
				{
					var word8 = this.maps.map_bg[(j + 15) >> 5][(k + 31) >> 5];
					if(word8 == 10 || word8 == 13)
						k = ((k + 31) >> 5) * 32 - 32;
					else
					if(word8 == 11 || word8 == 15)
						k = ((k + 31) >> 5) * 32;
				}
				j += 3;
				...
				} else
				{
					...
					{
						j = (j >> 5) * 32;
						var word9 = this.maps.map_bg[(j + 15) >> 5][(k + 31) >> 5];
						if(word9 >= 10 && word9 <= 15)
							j += 16;
						characterobject.c = 125;
					}
					var word10 = this.maps.map_bg[(j + 31) >> 5][(k + 31) >> 5];
					if(word10 >= 20)
					{
						j = ((j + 31) >> 5) * 32 - 32;
			...
			case 150: 
				if(characterobject.vx < 0 && (this.maps.getBGCode(j, k) >= 20 || this.maps.getBGCode(j, k + 31) >= 20))
				{
					j = (j >> 5) * 32 + 32;
					characterobject.vx = 0;
				}
				if(characterobject.vx > 0 && (this.maps.getBGCode(j + 31, k) >= 20 || this.maps.getBGCode(j + 31, k + 31) >= 20))
				{
					j = ((j + 31) >> 5) * 32 - 32;
					characterobject.vx = 0;
				}
			...
			case 210: 
				...
				if(k >= this.ochiru_y)
					characterobject.c = 0;
				if(characterobject.vy == 0 && (Math.abs(this.co_j.x - j) > 32 || k <= this.co_j.y))
					this.mSet(j, k, 100);
			...
			case 300: 
				...
				if(this.co_j.x >= j - 256 && this.co_j.x <= j + 256)
			...
			case 410: 
				...
				} else
				{
					...
					{
						j = ((j + 31) >> 5) * 32;
						var word11 = this.maps.map_bg[(j + 15) >> 5][(k + 31) >> 5];
						if(word11 >= 10 && word11 <= 15)
							j -= 15;
					...
					{
						var word12 = this.maps.map_bg[j >> 5][(k + 31) >> 5];
						if(word12 >= 20)
						{
							j = (j >> 5) * 32 + 32;
			...
			case 415: 
				...
				} else
				{
					...
					{
						j = (j >> 5) * 32;
						var word14 = this.maps.map_bg[(j + 15) >> 5][(k + 31) >> 5];
						if(word14 >= 10 && word14 <= 15)
							j += 16;
						characterobject.c = 425;
					}
					var word15 = this.maps.map_bg[(j + 31) >> 5][(k + 31) >> 5];
					if(word15 >= 20)
					{
						j = ((j + 31) >> 5) * 32 - 32;
			...
			case 600: 
				...
				} else
				{
					...
					} else
					{
						...
						if(this.maps.getBGCode(j, k + 31) >= 20 || this.maps.getBGCode(j, k + 8) >= 20)
						{
							j = (j >> 5) * 32 + 32;
			...
			case 605: 
				...
				} else
				{
					...
					if(this.maps.getBGCode(j + 31, k + 31) >= 20 || this.maps.getBGCode(j + 31, k + 8) >= 20)
					{
						j = ((j + 31) >> 5) * 32 - 32;
			...
			case 700: 
				if(!this.yachamo_cf)
					...
					if(this.co_j.x >= j - 256 && this.co_j.x <= j + 192 && Math.abs(this.co_j.x - j) >= 96)
						characterobject.c1 = 1;
			...
			case 1000: 
				...
				if(this.maps.getBGCode(j - 16, k) >= 20 || this.maps.getBGCode(j - 16, k + 31) >= 20)
				{
					j = ((j - 16) >> 5) * 32 + 32 + 16;
					characterobject.c = 1005;
				}
			...
			case 1100: 
				...
				if(Math.abs(this.co_j.x - j) <= 144 && Math.abs(this.co_j.y - k) <= 144)
				{
					characterobject.c1 = 1;
			...
			} else
			{
				...
				{
					if(this.co_j.c >= 100 && this.co_j.c < 200 && i4 < 30 && Math.abs(k - this.co_j.y) < 23)
						if(this.j_v_c > 0)
						{
							if(this.co_j.muki == 1)
							{
						...
						if(characterobject.c >= 400 && characterobject.c < 500 || characterobject.c >= 1000 && characterobject.c < 1200)
							this.jShinu(2);
						else
						if(i4 < 27 && this.co_j.vy > 0 && !this.j_mizu_f || flag)
						{
...
MainProgram.prototype.mSet = function(i, j, k)
{
	for(var l = 0; l <= 23; l++)
	{
		...
		case 100:
			...
			if(!this.dengeki_mkf && characterobject.c > 0 && this.maps.getBGCode(characterobject.x + 15, characterobject.y + 15) == 4)
			{
				this.m_kazu--;
				characterobject.c = 0;
			}
	...
	for(var i = 0; i <= 23; i++)
		if(this.co_m[i].c != 0)
		{
			...
			case 60:
				...
				var word0 = this.maps.map_bg[(characterobject.x + 11) >> 5][j2];
				if(word0 <= 3 || word0 >= 20)
					characterobject.c = 0;
			...
			case 90:
				if(characterobject.c2 < 48)
				{
					...
					if(characterobject.c2 >= 48)
					{
						...
						if(l3 < 32 || this.co_j.c < 100 || this.co_j.c >= 200)
						{
			...
			case 100:
				...
				if(this.maps.map_bg[l1][(characterobject.y + 12) >> 5] >= 20 || this.maps.map_bg[l1][(characterobject.y + 18) >> 5] >= 20)
					characterobject.c = 0;
			...
			case 300: 
				...
				if(this.maps.map_bg[(characterobject.x + 8) >> 5][j3] >= 20 || this.maps.map_bg[(characterobject.x + 23) >> 5][j3] >= 20)
					characterobject.c = 0;
				if(characterobject.x <= this.maps.wx - 32 || characterobject.x >= this.maps.wx + 512 + 128)
					characterobject.c = 0;
				if(this.g_c1 == 0)
			...
			case 400: 
				...
				if(this.maps.map_bg[i2][(characterobject.y + 15) >> 5] >= 20)
					characterobject.c = 0;
				if(this.g_c1 == 0)
			...
			case 700: 
				...
				if(characterobject.y <= this.maps.wy - 64 || characterobject.y >= this.maps.wy + 320 + 64)
					characterobject.c = 0;
				var l = this.maps.getBGCode(characterobject.x + 15, characterobject.y + 15);
			...
			if(this.co_j.c >= 100 && this.co_j.c < 200)
				...
				if(characterobject.c >= 100 && Math.abs(characterobject.x - this.co_j.x) <= 23 && Math.abs(characterobject.y - this.co_j.y) <= 28)
					if(this.j_v_c > 0)
					{
						characterobject.c = 0;
						this.m_kazu--;
		...
		if(this.co_jm[i].c != 0)
		{
			...
			case 60:
				...
				for(var k = 0; k <= this.t_kazu; k++)
					if(this.co_t[k].c >= 100)
					{
						...
						if(characterobject2.x > characterobject.x - 12 && characterobject2.x < characterobject.vx + 12 && Math.abs(characterobject2.y - characterobject.y) < 24)
						{
			...
			case 65:
				...
				for(var l = 0; l <= this.t_kazu; l++)
					if(this.co_t[l].c >= 100)
					{
						...
						if(characterobject3.x > characterobject.vx - 12 && characterobject3.x < characterobject.x + 12 && Math.abs(characterobject3.y - characterobject.y) < 24)
						{
			...
			case 200: 
				...
				if(this.grenade_type != 5 && this.grenade_type != 6)
				{
					...
					if(characterobject.vy > 30)
						characterobject.vy = 30;
					characterobject.y += characterobject.vy;
					if(characterobject.vy > 0 && this.maps.getBGCode(characterobject.x + 15, characterobject.y + 31) >= 20)
					{
						characterobject.c = 50;
						characterobject.y = ((characterobject.y + 31) >> 5) * 32 - 16;
						characterobject.c1 = 1;
				...
				if(this.maps.getBGCode(characterobject.x + 15, characterobject.y + 15) >= 20)
				{
					characterobject.c = 50;
					characterobject.c1 = 1;
					characterobject.c2 = 20;
			...
			if(characterobject.c >= 100)
			{
				for(var i1 = 0; i1 <= this.t_kazu; i1++)
				{
					...
					if(Math.abs(characterobject.x - characterobject4.x) >= 22 || Math.abs(characterobject.y - characterobject4.y) >= 22)
						continue;
					if(characterobject.vx < 0)
					{
...
MainProgram.prototype.aMove = function()
{
	...
	for(var i = 0; i <= this.a_kazu; i++)
	{
		var characterobject = this.co_a[i];
		if(characterobject.c != 0)
			if(this.maps.wx <= characterobject.c1 || this.maps.wx >= characterobject.c2)
			{
			...
			} else
			{
				...
				{
				case 60:
					if(k > this.maps.wx - 80 && k < this.maps.wx + 512 && l > this.maps.wy - 96 && l < this.maps.wy + 320)
					{
				...
				case 70:
					...
					if(characterobject.c3 >= 360)
						characterobject.c3 -= 360;
					if(k > this.maps.wx - 150 && k < this.maps.wx + 512 + 150 && l > this.maps.wy - 150 && l < this.maps.wy + 320 + 150)
					{
						characterobject.gf = true;
						this.a_hf = true;
						characterobject.pt = 1100;
						...
						if(this.co_j.c >= 100 && this.co_j.c < 200 && this.j_v_c <= 0)
							if(Math.abs(l11 - l12) >= Math.abs(j12 - i13))
							{
								if(this.co_j.x + 15 > l11 && this.co_j.x + 15 < l12)
								{
									...
									var k7 = i13 - j12;
									var l10 = rounddown((j4 * k7) / i1) + j12;
									if(l10 > this.co_j.y - 12 && l10 < this.co_j.y + 44)
										this.jShinu(1);
								} else
								if(this.co_j.x + 15 > l12 && this.co_j.x + 15 < l11)
								{
									var j1 = l11 - l12;
									var k4 = (this.co_j.x + 15) - l12;
									var l7 = j12 - i13;
									var i11 = rounddown((k4 * l7) / j1) + i13;
									if(i11 > this.co_j.y - 12 && i11 < this.co_j.y + 44)
										this.jShinu(1);
								}
							} else
							if(this.co_j.y + 15 > j12 && this.co_j.y + 15 < i13)
							{
								var k1 = i13 - j12;
								var l4 = (this.co_j.y + 15) - j12;
								var i8 = l12 - l11;
								var j11 = rounddown((l4 * i8) / k1) + l11;
								if(j11 > this.co_j.x - 12 && j11 < this.co_j.x + 44)
									this.jShinu(1);
							} else
							if(this.co_j.y + 15 > i13 && this.co_j.y + 15 < j12)
							{
								var l1 = j12 - i13;
								var i5 = (this.co_j.y + 15) - i13;
								var j8 = l11 - l12;
								var k11 = rounddown((i5 * j8) / l1) + l12;
								if(k11 > this.co_j.x - 12 && k11 < this.co_j.x + 44)
									this.jShinu(1);
							}
					} else
					{
				...
				case 80:
					if(k > this.maps.wx - 32 && k < this.maps.wx + 512 && l > this.maps.wy - 32 && l < this.maps.wy + 320)
					{
						characterobject.gf = true;
						this.a_hf = true;
				...
				case 100:
					if(k > this.maps.wx - 80 - 32 && k < this.maps.wx + 512 + 32 && l > this.maps.wy - 16 && l < this.maps.wy + 320 + 32)
					{
						characterobject.gf = true;
						this.a_hf = true;
						if(this.co_j.c == 100 && this.co_j.x + 15 >= k && this.co_j.x <= k + 64 && this.co_j.y == l - 32)
							this.j_a_id = i;
					} else
					{
						characterobject.gf = false;
					...
					if(this.co_j.c == 100)
					{
						...
						if(this.co_j.x + 15 >= k && this.co_j.x <= k + 64 && this.co_j.y + 31 >= l && this.co_j.y <= l + 13)
					...
					if(this.co_j.c == 100)
					{
						if(this.j_a_id == i)
						{
							...
							if(characterobject.vx > 0)
							{
								...
								if(this.maps.map_bg[i2][j5] >= 20 || this.maps.map_bg[i2][k8] >= 20)
							...
							if(characterobject.vx < 0)
							{
								...
								if(this.maps.map_bg[j2][k5] >= 20 || this.maps.map_bg[j2][l8] >= 20)
						...
						if(this.co_j.x + 15 >= k && this.co_j.x <= k + 64 && this.co_j.y + 31 >= l && this.co_j.y <= l + 13)
							if(characterobject.vx > 0)
							{
								...
								if(this.maps.map_bg[k2][l5] >= 20 || this.maps.map_bg[k2][i9] >= 20)
								{
									this.co_j.c = 230;
							...
							} else
							{
								...
								if(this.maps.map_bg[l2][i6] >= 20 || this.maps.map_bg[l2][i6] >= 20)
								{
									this.co_j.c = 230;
				...
				case 120:
					...
					if(this.co_j.c == 100)
					{
						if(this.j_a_id == i)
							this.co_j.y += k13;
						if(k13 < 0 && this.maps.getBGCode(this.co_j.x + 15, this.co_j.y) >= 20)
						{
							this.co_j.y = (this.co_j.y >> 5) * 32 + 16;
							this.jShinu(3);
						}
					...
					if(this.co_j.c == 100)
					{
						if(this.j_a_id == i)
						{
							...
							if(j13 > 0)
							{
								...
								if(this.maps.map_bg[i3][j6] >= 20 || this.maps.map_bg[i3][k9] >= 20)
							...
							if(j13 < 0)
							{
								...
								if(this.maps.map_bg[j3][k6] >= 20 || this.maps.map_bg[j3][l9] >= 20)
						...
						if(this.co_j.x + 15 >= k && this.co_j.x <= k + 64 && this.co_j.y + 31 >= l && this.co_j.y <= l + 13)
							if(j13 > 0)
							{
								...
								if(this.maps.map_bg[k3][l6] >= 20 || this.maps.map_bg[k3][i10] >= 20)
								{
									this.co_j.c = 230;
							...
							} else
							{
								...
								if(this.maps.map_bg[l3][i7] >= 20 || this.maps.map_bg[l3][j10] >= 20)
								{
									this.co_j.c = 230;
				...
				case 300: 
					if(k > this.maps.wx - 96 && k < this.maps.wx + 512 && l > this.maps.wy - 64 && l < this.maps.wy + 320)
					{
						characterobject.gf = true;
						this.a_hf = true;
						if(this.co_j.c == 100 && this.co_j.x + 15 >= k && this.co_j.x <= k + 48 && this.co_j.y == l - 32)
						{
							this.j_a_id = i;
							if(this.co_j.x + 15 >= k + 24 && this.co_j.x <= (k + 48) - 24 && this.co_j.y == l - 32 && characterobject.c4 != 1 && (this.sl_step == 0 || this.sl_step == 1 || this.sl_step == 10 || this.sl_step == 11 || this.dokan_mode != 2))
							{
								this.co_j.c1 = -10;
				...
				case 400: 
					if(k > this.maps.wx - 96 && k < this.maps.wx + 512 && l > this.maps.wy - 64 && l < this.maps.wy + 320)
					{
						characterobject.gf = true;
						this.a_hf = true;
						if(this.co_j.c == 100 && this.co_j.x + 15 >= k && this.co_j.x <= k + 80 && this.co_j.y == l - 32)
							this.j_a_id = i;
					} else
					{
						characterobject.gf = false;
					...
					if(characterobject.c3 == 0)
					{
						if(k - 64 < this.co_j.x + 15 && k + 96 + 64 > this.co_j.x + 15)
							characterobject.c3 = 1;
					} else
					...
					if(characterobject.c3 == 200 && (l -= 3) <= characterobject.vy)
					{
						l = characterobject.vy;
						characterobject.c3 = 0;
						if(k <= this.co_j.x + 15 && k + 96 > this.co_j.x + 15)
							characterobject.c3 = 100;
					}
					if(this.co_j.c == 100)
					{
						...
						if(this.co_j.x + 15 >= k && this.co_j.x <= k + 80 && this.co_j.y + 31 >= l && this.co_j.y <= l + 63)
							if(characterobject.c3 == 100)
							{
								this.co_j.y = l + 64;
								if(this.maps.getBGCode(this.co_j.x + 15, this.co_j.y + 31) >= 18)
				...
				case 500: 
					if(k > this.maps.wx - 96 - 32 && k < this.maps.wx + 512 + 32 && l > this.maps.wy - 32 && l <= this.maps.wy + 368)
					{
						characterobject.gf = true;
						this.a_hf = true;
						if(this.co_j.c == 100 && this.co_j.x + 15 >= k && this.co_j.x <= k + 80 && this.co_j.y == l - 32)
				...
				case 600: 
					if(k > this.maps.wx - 96 && k < this.maps.wx + 512 && l > this.maps.wy - 64 && l < this.maps.wy + 320)
					{
						characterobject.gf = true;
						this.a_hf = true;
						if(this.co_j.c == 100 && this.co_j.x + 15 >= k && this.co_j.x <= k + 48 && this.co_j.y == (l + 16) - 32)
							this.j_a_id = i;
					} else
					{
						characterobject.gf = false;
					...
					k += characterobject.vx;
					if(this.maps.getBGCode(k + 63 + 16, l) >= 20 || this.maps.getBGCode(k + 63 + 16, l + 32) >= 20)
					{
					...
					if(this.co_j.c == 100)
					{
						if(this.j_a_id == i)
						{
							...
							if(characterobject.vx > 0)
							{
								...
								if(this.maps.map_bg[i4][j7] >= 20 || this.maps.map_bg[i4][k10] >= 20)
									this.co_j.x = i4 * 32 - 16;
							}
						}
						if(this.co_j.x + 15 >= k && this.co_j.x <= k + 48 && this.co_j.y + 31 >= l + 16 && this.co_j.y <= l + 47)
							this.co_j.x = k + 49;
					}
					break;
				...
				case 700: 
					if(k > this.maps.wx - 96 && k < this.maps.wx + 512 && l > this.maps.wy - 64 && l < this.maps.wy + 320)
					{
						...
						if(this.co_j.c == 100 && this.co_j.x + 15 >= k && this.co_j.x <= k + 16)
							...
							if(characterobject.c3 >= 3 && characterobject.c3 <= 4 && this.co_j.y == (l - 32) + 16)
								this.j_a_id = i;
							else
							if(characterobject.c3 >= 5 && characterobject.c3 <= 6 && this.co_j.y == (l - 32) + 10)
								this.j_a_id = i;
							else
							if(this.co_j.y == l - 32)
								this.j_a_id = i;
				...
				if(this.jm_kazu > 0)
				{
					for(var j = 0; j <= 1; j++)
						if(this.co_jm[j].c >= 100)
						{
							...
							if(characterobject.c < 200)
							{
								if(characterobject1.x + 15 >= k && characterobject1.x <= k + 64 && characterobject1.y + 29 >= l && characterobject1.y + 2 <= l + 13)
							...
							if(characterobject.c == 300)
							{
								if(characterobject1.x + 15 >= k && characterobject1.x <= k + 48 && characterobject1.y + 29 >= l && characterobject1.y + 2 <= l + 31)
									if(characterobject1.c == 200)
									{
										characterobject1.c = 50;
										characterobject1.c1 = 1;
							...
							if(characterobject.c == 400)
							{
								if(characterobject1.x + 15 >= k && characterobject1.x <= k + 80 && characterobject1.y + 29 >= l && characterobject1.y + 2 <= l + 63)
									if(characterobject1.c == 200)
									{
										characterobject1.c = 50;
										characterobject1.c1 = 1;
							...
							if(characterobject.c == 500 && characterobject1.x + 15 >= k && characterobject1.x <= k + 80 && characterobject1.y + 29 >= l && characterobject1.y + 2 <= l + 13)
								if(characterobject1.c == 200)
								{
									characterobject1.c = 50;
									characterobject1.y = l - 8;
...
MainProgram.prototype.bMove = function()
{
	...
	if(this.co_j.c >= 100 && this.co_j.c < 200 && this.co_b.c >= 100 && Math.abs(this.co_b.x - this.co_j.x) < 42 && this.co_j.y > this.co_b.y - 20 && this.co_j.y < this.co_b.y + 40)
		if(this.co_b.pt == 1250 || this.co_b.pt == 1255)
			this.jShinu(2);
		else
		if(Math.abs(this.co_b.x - this.co_j.x) < 34 && this.co_j.vy > 0)
		{
			if(this.co_b.c < 200)
			{
				this.co_b.c4--;
