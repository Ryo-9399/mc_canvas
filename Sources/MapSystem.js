		...
		for (var i = 0; i <= 16; i++) {
			...
			if (n > 0) {
				this.gg.drawPT2(
					32 + i * 32,
					32 + j * 32,
					this.map_bg[this.os2_wx + i][this.os2_wy + j]
				);
			}
		}
	}
...
MapSystem.prototype.drawMapLayer = function(
	paramInt1,
	paramInt2,
	paramInt3,
	paramInt4,
		...
		for (j = 0; j <= 10; j++) {
			for (i = 0; i <= 16; i++) {
				...
				if (n == 7) {
					...
					if (this.gg.layer_mode != 2) {
						if (
							this.map_bg[this.os2_wx + i - 1][this.os2_wy + j] ==
							4
						) {
					...
					if (
						(this.mp.clear_type != 2 && this.mp.clear_type != 3) ||
						this.mp.coin_kazu <= 0
					) {
						if (
							this.mp.stage_max >= 2 &&
							this.mp.stage >= this.mp.stage_max
						) {
							if (paramInt3 >= 2) {
								n = 98;
							} else {
					...
					if (
						this.mp.map_data_option[this.os2_wx + i][
							this.os2_wy + j
						] == 1
					) {
						n = 0;
						...
						this.map_bg[this.os2_wx + i][this.os2_wy + j - 1] == 4
					) {
						this.gg.drawPT2(32 + i * 32, 32 + j * 32, 4);
						this.gg.drawPT2(32 + i * 32, 32 + j * 32, n);
		...
		if (nx > this.os2_wx) {
			...
			for (var i = 0; i <= 9; i++) {
				if (this.map_bg[this.os2_wx][this.os2_wy + i] > 0) {
					this.g2.drawImage(
						this.hi[this.map_bg[this.os2_wx][this.os2_wy + i]],
						32,
						32 + i * 32,
						this.ap
					);
		...
		if (nx > this.os2_wx) {
			...
			for (var i = 0; i <= 16; i++) {
				if (this.map_bg[this.os2_wx + i][this.os2_wy] > 0) {
					this.g2.drawImage(
						this.hi[this.map_bg[this.os2_wx + i][this.os2_wy]],
						32 + i * 32,
						32,
						this.ap
					);
	...
	for (var i = 0; i <= 10; i++) {
		for (var j = 0; j <= 16; j++) {
			switch (this.map_bg[this.os2_wx + j][this.os2_wy + i]) {
				case 5:
					if (
						this.map_bg[this.os2_wx + j][this.os2_wy + i - 1] == 4
					) {
				...
				case 8:
					if (paramInt == 0) {
						if (
							this.map_bg[this.os2_wx + j - 1][this.os2_wy + i] ==
							4
						) {
							this.gg.drawBG2(32 + j * 32, 32 + i * 32, 4);
							if (
								(this.mp.clear_type != 2 &&
									this.mp.clear_type != 3) ||
								this.mp.coin_kazu <= 0
							) {
								if (
									this.mp.stage_max >= 2 &&
						...
						this.map_bg[this.os2_wx + j - 1][this.os2_wy + i] == 4
					) {
						this.gg.drawBG2(32 + j * 32, 32 + i * 32, 4);
						if (
							(this.mp.clear_type != 2 &&
				...
				case 18:
					if (
						this.mp.map_data_option[this.os2_wx + j][
							this.os2_wy + i
						] == 1
					) {
						if (
							this.map_bg[this.os2_wx + j][this.os2_wy + i - 1] ==
							4
						) {
							this.gg.drawBG2(32 + j * 32, 32 + i * 32, 4);
						} else {
...
MapSystem.prototype.getBGCode = function(paramInt1, paramInt2) {
	return this.map_bg[rightShiftIgnoreSign(paramInt1, 5)][
		rightShiftIgnoreSign(paramInt2, 5)
	];
};
...
MapSystem.prototype.putBGCode = function(paramInt1, paramInt2, paramInt3) {
	...
	if (
		...
		this.gg.drawBG2(
			(paramInt1 - this.os2_wx) * 32 + 32,
			(paramInt2 - this.os2_wy) * 32 + 32,
			paramInt3
		);
	}
};
