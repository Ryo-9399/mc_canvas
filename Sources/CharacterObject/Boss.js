...
class Boss extends CharacterObject {
		...
		if (mp.boss2_type === 2) {
			...
			for (const [count, degree] of zip(attack_count, degrees)) {
				...
				for (const rad of rads) {
					const cos = rounddown(Math.cos(rad) * 12);
					const sin = rounddown(Math.sin(rad) * 10);
	...
	boss2BubbleBeam(mp, degree) {
		for (let i = 0; i < 8; i++) {
			...
			const cos = rounddown(Math.cos(d) * 8);
			const sin = -rounddown(Math.sin(d) * 8);
	...
	boss2Attack6(mp, direction) {
		...
		if (rad !== null) {
			const cos = rounddown(Math.cos(rad) * 12);
			const sin = rounddown(Math.sin(rad) * 8);
