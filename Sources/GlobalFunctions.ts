...
// ゲームパッド16ボタンの入力状態保持
const gamepadInput = Array(16).fill(false);
...
*/
	...
	__loop() {
		...
		if (navigator.getGamepads) {
			// ブラウザ対応確認
			// フォーカスを得ていないと動作しない
			if (Game.focus.hasFocus(this.__mc)) {
				// getGamepads()の戻り値はnullを含んだ配列のようなオブジェクト（配列とは限らない）
				const gamepads = Array.prototype.filter.call(navigator.getGamepads(), (p) => p);
				const gamepad = gamepads[0];

				if (gamepad) {
					for (let i = 0; i < gamepad.buttons.length; i++) {
						if (i >= gamepadInput.length) break; // 想定入力数の範囲外の入力は無視
						if (Game.gamepadMap[i] === 0) continue; // キーコード0はスキップ

						const { pressed } = gamepad.buttons[i];
						if (!gamepadInput[i] && pressed) {
							this.__mc.gk.keyPressed({
								keyCode: Game.gamepadMap[i],
								preventDefault: () => {},
							});
						} else if (gamepadInput[i] && !pressed) {
							this.__mc.gk.keyReleased({
								keyCode: Game.gamepadMap[i],
								preventDefault: () => {},
							});
						}
						gamepadInput[i] = pressed;
					}
				}
			}
		}
	...
	/**
	 * ゲームパッドとキー入力の対応表。ボタンに対応するキーコードを格納する。対応なしの場合は0を代入する。
	 *
	 * 参照: https://w3c.github.io/gamepad/#dfn-standard-gamepad-layout
	 *
	 * インデックス0から順に
	 * 右手 下(A), 右(B), 左(X), 上(Y),
	 * L1(LB), R1(RB), L2(LT), R2(RT),
	 * 中央 左(select/back), 右(start/forward), L3(Lアナログパッド押し込み), R3(Rアナログパッド押し込み)
	 * 左手 上(十字↑), 下(十字↓), 左(十字←), 右(十字→)
	 *
	 * ↓↓↓ 初期配置 ↓↓↓
	 *
	 * * 右手 下(A) → Zキー (ジャンプ)
	 * * 右手 右(B) → Xキー (攻撃)
	 * * 中央 左(select/back) → Pキー (ポーズ)
	 * * 中央 右(start/forward) → Tキー (タイトル画面へ戻る)
	 * * 左手 十字ボタン → 方向キー (移動とか)
	 * * それ以外 → 対応なし
	 */
	static gamepadMap = [90, 88, 0, 0, 0, 0, 0, 0, 80, 84, 0, 0, 38, 40, 37, 39];
}
